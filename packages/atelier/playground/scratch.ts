...
// // console.log(check);
// // playground/scratch.ts
// import { hexToRgb } from "../src/transform/presets/hexToRgb";
// import { pxToRem } from "../src/transform/presets/pxToRem";
// import { applyTransforms } from "../src/transform/transform";
// import { ResolvedToken } from "../src/types";
// // ============================================================
// // J) applyTransforms — pipeline real con pxToRem + hexToRgb
// // ============================================================
...
//             path: "color.brand",
//             type: "color",
//             value: { colorSpace: "srgb", components: [0, 0, 0], hex: "#ff000080" }, // 8 dígitos, alpha ~0.5
//             references: [],
//         },
//     ],
//     [
...
// const result = applyTransforms(tokens, [pxToRem, hexToRgb]);

// console.log("spacing.md (esperado: 1rem, transformado por pxToRem):", result.get("spacing.md"));
// console.log("color.brand (esperado: components desde hex, alpha ~0.5):", result.get("color.brand"));
// console.log(
//     "font.weight.bold (esperado: SIN CAMBIOS, ningún transform le aplica):",
//     result.get("font.weight.bold"),
// );
// playground/scratch.ts
import { buildTokens } from "../src/build"; // ajusta el path real
import { pxToRem } from "../src/transform/presets/pxToRem";

// ============================================================
// K) buildTokens — pipeline completo de extremo a extremo
// ============================================================

const testDoc = JSON.stringify({
    spacing: {
        // Sano — debe llegar a "tokens", con pxToRem aplicado (16px -> 1rem)
        md: { $type: "dimension", $value: { value: 16, unit: "px" } },
    },
    color: {
        // Sano — debe llegar a "tokens", sin cambios (pxToRem no le aplica)
        brand: { $type: "color", $value: { colorSpace: "srgb", components: [1, 0, 0] } },
    },
    broken: {
        // Ciclo — nunca llega a checkTokenShape, debe ir a "errors" con code 'cycle'
        a: { $type: "dimension", $value: "{broken.b}" },
        b: { $type: "dimension", $value: "{broken.a}" },
    },
    invalid: {
        // Estructuralmente resoluble, pero forma inválida — debe ir a "errors"
        // con code 'invalid-shape' (value no es un number)
        weird: { $type: "dimension", $value: { value: "not-a-number", unit: "px" } },
    },
});

const { tokens, errors } = buildTokens(testDoc, [pxToRem]);

console.log("=== tokens (esperado: 2 entradas — spacing.md, color.brand) ===");
for (const [path, token] of tokens) {
    console.log(path, "→", token);
}

console.log("=== errors (esperado: 2 entradas — cycle + invalid-shape) ===");
console.log(errors);

console.log(`Total tokens: ${tokens.size} (esperado: 2)`);
console.log(`Total errors: ${errors.length} (esperado: 2, o más si el ciclo reporta 2 veces)`);
